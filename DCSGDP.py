...
def resample(args,model,resample_dataloader,optimizer,device,resample_num,pj_gau):
    ...
    for sample in split_dataloader:
        ...
        norm_stack=torch.cat([norm_stack,per_sample_norms],dim=0)
        optimizer.zero_grad()
        model.zero_grad()
            
    optimizer.mean_norm=torch.mean(norm_stack)
    optimizer.max_grad_norm=adaptive.histgram_clip(args,norm_stack,args.sigma_t,args.percentile,args.bins,optimizer.max_grad_norm,mode="Gaussian",step=optimizer.control.resample_steps,save=False)
...
def train(args, model, train_loader,resample_dataloader ,optimizer, privacy_engine, epoch, device, resample_num, project_gaussian,project2,loss_rec,mean_norm_rec):
    ...
    while True:
        ...
        optimizer=resample(args,model,resample_dataloader,optimizer,device,resample_num,project_gaussian)
        print("-----")
        print(mgn)
    ...
    if args.dp_able:
        ...
        rdp=optimizer.control.compute(2000*args.delta,2)
        eps, best_alpha1 = privacy_analysis.get_privacy_spent(
            orders=optimizer.control.DEFAULT_ALPHAS, rdp=rdp, delta=args.delta
        )
        total=rdp1+rdp
